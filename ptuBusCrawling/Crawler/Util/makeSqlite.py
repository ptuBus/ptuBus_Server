...
class makeSqlite:
    ...
    def makeDataBase(self):
        self.cur.execute("CREATE TABLE IF NOT EXISTS BusTerminal (id integer PRIMARY KEY, startStationName text, startStationID text,"
                         "endStationName text,endStationID text, isExpress integer)")
        self.cur.execute("CREATE TABLE IF NOT EXISTS TrainStation (id integer PRIMARY KEY, startStationName text, startStationID text,"
                         "endStationName text,endStationID text)")
        self.cur.execute("CREATE TABLE IF NOT EXISTS SubwayLine (id integer PRIMARY KEY, lineName text, lineCode text, lineColorCode text,"
                         "lineSaidName text)")
        self.cur.execute("CREATE TABLE IF NOT EXISTS SubwayStation (id integer PRIMARY KEY, stationName text, stationCode text,"
    ...
    def makeData(self):
        ...
        for name in self.URL:
            data = self.openURL(name)
            if (name == 'BusTerminal'):
                self.cur.executemany('INSERT INTO BusTerminal VALUES (?, ?, ?, ?, ?, ?)', data)
            elif (name == 'TrainStation'):
                self.cur.executemany('INSERT INTO TrainStation VALUES (?, ?, ?, ?, ?)', data)
            elif (name == 'SubwayLine'):
                self.cur.executemany('INSERT INTO SubwayLine VALUES (?, ?, ?, ?, ?)', data)
            elif (name == 'SubwayStation'):
                self.cur.executemany('INSERT INTO SubwayStation VALUES (?, ?, ?, ?, ?)', data)
        self.conn.commit()
        fileKey = "{0}{1}{2}{3}{4}{5}".format(now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec)
        AppDBModel(
            fileKey=fileKey
            ).save()

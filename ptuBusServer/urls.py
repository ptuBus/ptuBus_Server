...
urlpatterns = [
    ...
    re_path(
        r"^bus/(?P<startStationID>.+)(&P<endStationID>.+)(&P<schedule>\d+)/$",
        BusTimeTableListView.as_view(),
    ),
    re_path(
        r"^bus/(?P<startStationID>.+)(&P<endStationID>.+)/$",
        BusTimeTableListView.as_view(),
    ),
    re_path(r"^bus/$", BusTimeTableListView.as_view()),
    path("bus/terminal/", BusTerminalListView.as_view()),
    re_path(
        r"^schoolbus/(?P<startStationID>.+)(&P<endStationID>.+)(&P<upDownTypeCode>.+)(&P<schedule>\d+)/$",
        SchoolBusTimeTableListView.as_view(),
    ),
    re_path(
        r"^schoolbus/(?P<startStationID>.+)(&P<endStationID>.+)(&P<upDownTypeCode>.+)/$",
        SchoolBusTimeTableListView.as_view(),
    ),
    re_path(r"^schoolbus/$", SchoolBusTimeTableListView.as_view()),
    re_path(
        r"^train/(?P<runDay>.+)(&P<endStationID>.+)(&P<trainClass>[1-3]+)(&P<schedule>\d+)/$",
        TrainTimeTableListView.as_view(),
    ),
    re_path(
        r"^train/(?P<runDay>.+)(&P<endStationID>.+)(&P<trainClass>[1-3]+)/$",
        TrainTimeTableListView.as_view(),
    ),
    re_path(
        r"^train/(?P<runDay>.+)(&P<endStationID>.+)(&P<schedule>\d)/$",
        TrainTimeTableListView.as_view(),
    ),
    re_path(
        r"^train/(?P<runDay>.+)(&P<endStationID>.+)/$", TrainTimeTableListView.as_view()
    ),
